...
class PublicationCard extends Element {
	...
	// Safely read link from either plain objects or class instances
	getLink(idx) {
		const item = this.fileLinks?.[idx];
	...
	toHtml() {
		...
		// cite link is typically second item
		const citeText = this.getLink(1);
		if (citeText !== "") {
			answer +=
				'<a class="cite-btn" onclick="copy_cite(\'' +
				this.title.replaceAll("'", "").replaceAll(" ", "_") +
				'\');">' + CITE_SYMBOL + 'Cite</a></div>';
		} else {
			answer += '</div>';
		}

		answer += '<h4>' + (this.authors ?? "") + "<br>" + (this.publisher ?? "") + '</h4>';
		answer += descriptionTrim(this.description ?? "");
		answer += '<div class="personal-row space-between align-items-center mobile-row-breaker">';

		if (window.screen.width > 400) {
			answer +=
				'<div class="publication-details"><span>' +
				(this.publicationStatus ?? "") + '</span><span>' +
				(this.year ?? "") + '</span><span>' +
				(this.type ?? "") + '</span></div>';
		}

		// download link is typically first item
		const dl = this.getLink(0);
		if (dl !== "" && this.isFileFormat(dl)) {
			answer += '<div class="inner-publication-card-div">';
			answer += '<a href="' + dl + '" class="download-btn acadmic-card-margin-fix acadmic-download-btn">Download</a>';
			answer += '<a href="' + dl + '" class="read-online-btn acadmic-card-margin-fix">Read Online</a>';
			answer += '</div>';
		}

		answer += '</div>';

		// hidden input used by copy_cite()
		answer +=
	...
	// ✅ FIXED SIGNATURE (no trailing comma)
	isFileFormat(str) {
		const arr = [".pdf", ".htm", ".html", ".ppt", ".pptx", ".xls", ".xlsx", ".doc", ".docx", ".txt"];
		for (const suffix of arr) {
			if (str.length > suffix.length && str.toLowerCase().endsWith(suffix)) return true;
		}
		return false;
	}
	...
	static createListFromJson(jsonObj) {
		...
		for (let i = 0; i < jsonObj.length; i++) {
			answer.push(PublicationCard.createFromJson(jsonObj[i]));
		}
		return answer;
	}
	...
	static filterList(objList, property, filterValue) {
		return objList.filter(x => x[property] == filterValue);
	}

	static splitByProperty(objList, property) {
		const answer = {};
		let spliter = objList[0][property];
		let subGroup = [objList[0]];

		for (let i = 1; i < objList.length; i++) {
			if (objList[i][property] != spliter) {
				answer[spliter] = [...subGroup];
				spliter = objList[i][property];
				subGroup = [objList[i]];
			} else {
				subGroup.push(objList[i]);
			}
		}
		answer[spliter] = [...subGroup];
		return answer;
